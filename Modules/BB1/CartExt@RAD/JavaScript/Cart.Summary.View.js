	...
	,	initialize: function initialize (options)
		{
			...
			this.viewapplication=this.options.application;
		}
,	events: {
   'click [data-action="startCheckout"]': 'startCheckout'
   }
   
   	,	startCheckout: function (e)
   {
   e.preventDefault();
var $confirm=$("#confirmBackorders");
if($confirm.length>0){
var confirmed=$confirm.is(':checked');
if(confirmed){
	document.location.href=SC.SESSION.touchpoints.checkout;
	return;
}
$('.modal').modal('hide');
var backorderView = new Backbone.View({application: this.viewapplication});

			backorderView.title = _('Your order contains backordered items').translate();
			backorderView.render = function ()
			{
				this.$el.append("<p>Please confirm your backordered items by ticking the 'Confirm your backorders' field on this page OR you can remove backordered items by clicking the 'Remove' buttons on out of stock items and/or reducing your quantity for item lines marked as backordered.</p><br /><p style='text-align:center;'><button class='button-medium button-secondary' data-dismiss='modal'>OK</button></p>");
			};

			backorderView.showInModal();
			}else{
			document.location.href=SC.SESSION.touchpoints.checkout;
			}
   }
...
var containsBackorders=false;
			lines.map(function (line)
				{
				console.log(line.get("item").get("quantity")+">"+line.get("item").get("quantityavailable"))
if(line.get("item").get("quantity")>line.get("item").get("quantityavailable")){
	containsBackorders=true;
}
				});
			...
			return {
                ...
                ,   showProceedButton: Configuration.get('siteSettings.sitetype') === 'STANDARD'
					...
					//TODO REMOVE THIS FROM HERE. this is a feature that should not be here!
					// @property {String} urlLogin
				,	urlLogin: this.getUrlLogin()
                	// @property {Boolean} isPriceEnabled
				,	isPriceEnabled: !ProfileModel.getInstance().hidePrices()
				,containsBackorders:containsBackorders
			};
