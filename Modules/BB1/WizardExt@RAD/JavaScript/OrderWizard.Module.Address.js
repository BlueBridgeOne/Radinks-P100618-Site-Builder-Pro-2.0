	...
	,	initialize: function ()
		{
			...
			filterAddress: function(e) //BB1 GT - Filter addresses using display.
			{
				...
				var self = this;
				var $list=$this.closest(".order-wizard-address-module-list-placeholder");
				$list.find(".address-details-info").each(function() {
					var text = ($(this).text() || "").toLowerCase();
					if (self.containsFilter(text, filters)) {
						$(this).closest(".order-wizard-address-cell12").css("display", "block");
					} else {
						$(this).closest(".order-wizard-address-cell12").css("display", "none");
					}
	...
	,	render: function (not_trigger_ready)
		{
			...
			{
				// if the user is a guest, and its editing the already submitted address
	...
	,	eventHandlersOn: function ()
		{
			...
			this.addresses
				...
				.on('add', function ()
				{
					this.setAddress(this.addresses.models[this.addresses.length-1].id);
			...
			this.trigger('ready', true);
				...
				}, this).on('reset sync remove change destroy', function ()
				{
					this.render(true);
				}, this)
		...
		,	'Address.List': function ()
			{
				...
				,	viewsPerRow: this.itemsPerRow || 1
	...
	,	getContext: function ()
		{
			...
			,	showSingleAddressDetails: !!((this.isSameAsEnabled && this.sameAs) || this.addressId && !this.address.isNew())
			...
			,showFilter:this.getAddressCollection().length>1
			};
		}
	});
});
